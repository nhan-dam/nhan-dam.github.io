# ML Workstation Setup — Mac mini M4 Pro + External NVMe
> Using `uv` for Python & environment management.
>
> Machine: Mac mini M4 Pro · 64 GB RAM · internal 512 GB SSD.
>
> External: Samsung 990 Pro 2 TB in OWC M.2 Express (Thunderbolt/USB4).
>
...
- `uv` uses per-project virtual environments — each project/competition has its own isolated `.venv`, stored inside the project folder on the external drive
...
> **Why APFS?** Native macOS filesystem — supports snapshots, efficient space sharing, and fast metadata operations. Better than exFAT for a Mac-only workflow.
>
...
### 1.2 Verify Mount Point

```bash
ls /Volumes/ML_Workspace
```

On a freshly formatted empty volume you'll see either blank output (normal) or a `.Spotlight-V100` folder that macOS creates automatically for search indexing. Both are fine.
...
Key fields to confirm in the full `diskutil info` output if you want a broader sanity check:
- `Device Location: External` — confirms it's the external drive, not your internal SSD
- `Solid State: Yes` and `Protocol: PCI-Express` — confirms it's the NVMe
- `SMART Status: Verified` — drive health is good
- `Volume Used Space:` — should be near zero after a fresh format

### 1.3 Verify Auto-Mount on Login

Reboot once and confirm `/Volumes/ML_Workspace` is present. APFS volumes auto-mount on macOS by default.

---

## Phase 2 — Directory Structure on External Drive
...
> **Key insight:** Each project's `.venv` lives inside the project folder on the external drive. The `uv` cache at `/Volumes/ML_Workspace/cache/uv` is shared across all projects via hard links — if two projects need the same PyTorch wheel, it's downloaded once and reused. This is much more space-efficient than conda's approach.

---

## Phase 3 — Core System Tools

### 3.1 Check for Xcode Command Line Tools
...
- If it returns a path like `/Library/Developer/CommandLineTools` — already installed, skip to 3.2
...
Accept the dialog. Takes ~5 minutes.

### 3.2 Install Homebrew

Homebrew is a package manager for macOS — it lets you install command-line tools with a single command rather than downloading installers manually. It's the standard way to get system-level tools that aren't Python packages and can't be installed via `uv`.

```bash
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
```

After installation, Homebrew will print two commands for you to run. Run both exactly as printed — they will look like:
...
> - `~/.zshrc` runs on every new shell instance — the right place for aliases and env vars (used in Phase 5)
>
...
### 3.3 Install System Dependencies via Homebrew
...
- `tree` — prints a visual directory structure (`tree /Volumes/ML_Workspace/projects`)
- `swig` — required by some Gymnasium environments (Box2D, etc.)
- `cmake` — required by some ML C-extension builds

> **Note on `curl`:** macOS ships with `curl` pre-installed and it's sufficient for all ML work. Adding Homebrew's `curl` to this list generates a "keg-only" warning — Homebrew installs it but deliberately keeps it out of your PATH to avoid conflicting with the system version. It adds noise without benefit, so leave it out.
...
## Phase 5 — Shell Environment Variables
...
# Ollama performance settings (Apple Silicon optimizations)
...
> - The single quotes around `'EOF'` are important: they prevent the shell from expanding variables like `$ML_ROOT` before writing them to the file. You want the literal variable references written to `~/.zshrc` so they're evaluated fresh each time a terminal opens.
> - The closing `EOF` signals the end of the block
>
> **`OLLAMA_FLASH_ATTENTION` and `OLLAMA_KV_CACHE_TYPE`** are Apple Silicon memory optimizations for Ollama:
> - `OLLAMA_FLASH_ATTENTION="1"` — enables Flash Attention, reducing memory usage during inference
> - `OLLAMA_KV_CACHE_TYPE="q8_0"` — quantizes the KV cache (temporary attention scratch space) to 8-bit instead of 16-bit, cutting its memory footprint roughly in half. This does **not** affect model weight precision — only the intermediate computation buffer. Output quality difference is imperceptible in practice.
>
...
>
> **Do this before installing any Python or packages.** These env vars ensure uv downloads Python and caches wheels to the external drive from the very first install.

---

## Phase 6 — Install Python via `uv`
...
## Phase 7 — Project-Based Workflow with `uv`
...
>
> **Note on `huggingface_hub`:** The CLI entry point shipped with this package was renamed from `huggingface-cli` to `hf` as of recent versions. The login command is now `uv run hf auth login` (see Phase 13). If you ever need to check what CLI commands a package registers, inspect `.venv/lib/python3.12/site-packages/<package>.dist-info/entry_points.txt`.

### 7.3 Reproducing the Environment on Another Machine

When you run `uv add`, two files are automatically maintained:

- **`pyproject.toml`** — your declared dependencies (e.g. `torch`, `trl`). The source of truth — human-readable, can be edited directly. `uv` always resolves `uv.lock` from this file, never the other way around.
...
`uv sync` reads `uv.lock` and recreates `.venv` with exactly the same versions — no resolution step needed, which is why it's fast. If the other machine is a different architecture (e.g. Linux x86 instead of Apple Silicon), `uv sync` re-resolves platform-specific wheels automatically while keeping all versions consistent where possible.

If you ever need a traditional `requirements.txt` for compatibility with tools that don't understand `uv.lock`:

```bash
uv pip freeze > requirements.txt
```

For your own projects between machines, `uv.lock` + `uv sync` is the better mechanism — it's more precise and fully automated.

### 7.4 Run Commands in the Project Environment
...
### 7.5 Repeat for Each New Project
...
Each project gets its own isolated `.venv`. Wheels are cached in `/Volumes/ML_Workspace/cache/uv` and hard-linked — no redundant downloads across projects.

---

## Phase 8 — Jupyter Lab Setup

### 8.1 When to Register a Jupyter Kernel
...
### 8.2 Switch Kernels in Notebooks
...
## Phase 9 — Editor: VS Code + Claude Code

### 9.1 Why Not Cursor?

Cursor ($20/month) is a popular AI-native IDE, but given you already have Claude Pro (which includes Claude Code), paying for Cursor is redundant for your use case:
...
Install the `code` CLI so you can control VS Code from the terminal:
...
### 9.4 Auto-Detection of uv Environments
...
> **`.vscode/`** — VS Code creates this folder inside project directories to store workspace-specific settings (chosen interpreter path, debug configs, extension recommendations). These are personal editor preferences that shouldn't be committed to git and imposed on collaborators who may use different editors or local paths.
...
Kaggle authenticates via two environment variables that you already set in Phase 5:
...
## Phase 12 — Ollama (Local LLM Inference)
...
### 12.2 Redirect Model Storage to External Drive

Background services launched by `brew services` run in a separate environment from your terminal and do not read `~/.zshrc`. This means the `OLLAMA_MODELS` path you set in Phase 5 is invisible to the service — models will default to `~/.ollama/models/` on your internal SSD.
...
## Phase 13 — HuggingFace & W&B Login
...
>
> **Where tokens are stored:** Both of the following paths are on your external NVMe because of the `HF_HOME` env var set in Phase 5. If you can't remember the paths, `echo $HF_HOME` points you to the right directory.
> - `$HF_HOME/token` — active token used by the HuggingFace library
> - `$HF_HOME/stored_tokens` — all named tokens you've logged in with
...
> ```
> `~/.netrc` is a standard Unix credentials file used by many CLI tools. Its permissions are `600` (owner read/write only), same as any credentials file.
...
conda's package channels offer no advantage for this stack — every library (PyTorch MPS, MLX, TRL, PEFT, Gymnasium) ships native Apple Silicon wheels on PyPI. `uv` is faster, simpler, and produces more reproducible environments.